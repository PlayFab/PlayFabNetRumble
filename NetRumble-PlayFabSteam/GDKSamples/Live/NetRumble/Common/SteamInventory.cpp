...
void Inventory::CheckForItemDrops()
{
	SteamInventory()->TriggerItemDrop(&m_playtimeRequestResult, SpaceWarItemTimedDropList);
}

void Inventory::GetAllItems()
{
	SteamInventory()->GetAllItems(nullptr);
}

void Inventory::OnSteamInventoryResult(SteamInventoryResultReady_t* callback)
{
...
void Inventory::OnSteamInventoryFullUpdate(SteamInventoryFullUpdate_t* callback)
{
	...
	{
		vecDetails.resize(count);
		bGetResult = SteamInventory()->GetResultItems(callback->m_handle, vecDetails.data(), &count);
	}
	...
	if (bGetResult)
	{
		...
		for (size_t i = 0; i < vecDetails.size(); i++)
		{
			DEBUGLOG("vecDetails[%d] Item ID in game is %d\n", i, vecDetails[i].m_iDefinition);
			tempItemID[vecDetails[i].m_iDefinition]++;
		}
