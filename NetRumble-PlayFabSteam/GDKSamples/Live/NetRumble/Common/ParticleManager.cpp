...
{
	std::shared_ptr<ParticleSystem> clone = std::make_shared<ParticleSystem>();
...
void ParticleSystem::GenerateParticles(float elapsedTime)
{
	...
	while (ReleaseTimer >= ReleaseRate)
	{
		...
		std::shared_ptr<Particle> particle = particles->GetNextParticle();
		if (particle == nullptr)
		{
			break;
		}
...
{
	std::shared_ptr<ParticleEffect> clone = std::make_shared<ParticleEffect>();
...
void ParticleEffectManager::RegisterParticleEffect(ParticleEffectType effectType, std::function<std::shared_ptr<ParticleEffect>(void) > effectFactory, int initialCount)
{
	...
	std::shared_ptr<ParticleEffect> effect = effectFactory();
	effect->Initialize();
	effect->Stop(true);
	...
	{
		std::shared_ptr<ParticleEffect> cloneEffect = effect->Clone();
