...
void UserStartupScreen::AcquireUserMenu()
{
	...
	m_menuEntries.emplace_back("LOGIN WITH STEAM",
		[this]()
		{
			m_menuEntries.clear();
			m_menuEntries.emplace_back("LOGIN WITH STEAM...");
...
void UserStartupScreen::LoginWithCustomId()
{
	...
	if (Managers::Get<OnlineManager>()->GetLoginType() == PlayFabLoginType::LoginWithSteam)
	{
		Managers::Get<ScreenManager>()->ShowError("Please login with Steam", []() {
...
void UserStartupScreen::LoginWithSteam()
{
	Managers::Get<GameStateManager>()->SwitchToState(GameState::Login);
	...
	Managers::Get<OnlineManager>()->LoginWithSteam(
		[this](bool success, const std::string& userId)
		{
			UNREFERENCED_PARAMETER(userId);
			if (success)
			...
			{
				DEBUGLOG("Unable to login with Steam ID");
			}
		});
}
