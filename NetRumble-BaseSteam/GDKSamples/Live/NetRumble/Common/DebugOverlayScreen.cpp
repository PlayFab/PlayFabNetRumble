...
void DebugOverlayScreen::Draw(float totalTime, float elapsedTime)
{
	...
	msgStr = "DeathCount : " + std::to_string(Managers::Get<SteamOnlineManager>()->GetDeathCount());
	scale = 0.50f * GetScaleMultiplierForViewport(viewportWidth, viewportHeight);
	renderContext->DrawString(
		spriteFont,
		msgStr.c_str(),
	...
	msgStr = "VictoryCount : " + std::to_string(Managers::Get<SteamOnlineManager>()->GetVictoryCount());
	scale = 0.50f * GetScaleMultiplierForViewport(viewportWidth, viewportHeight);
	renderContext->DrawString(
		spriteFont,
		msgStr.c_str(),
	...
	msgStr = "StartGameCount : " + std::to_string(Managers::Get<SteamOnlineManager>()->GetStartGameCount());
	scale = 0.50f * GetScaleMultiplierForViewport(viewportWidth, viewportHeight);
	renderContext->DrawString(
		spriteFont,
		msgStr.c_str(),
	...
	msgStr = "DebugLogMessage: " + g_game->m_OutputMessage;
	scale = 0.50f * GetScaleMultiplierForViewport(viewportWidth, viewportHeight);
	renderContext->DrawString(
		spriteFont,
		msgStr.c_str(),
