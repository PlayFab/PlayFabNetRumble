...
NetRumble::LeaderboardMenu::LeaderboardMenu()
{
...
void NetRumble::LeaderboardMenu::Draw(float totalTime, float elapsedTime)
{
	...
	else
	{
		...
		int displayUserCount = MaxLeaderboardDisplayUserCount < m_leaderBoardUser.size() ? MaxLeaderboardDisplayUserCount : m_leaderBoardUser.size();
		for (int i = 0; i < displayUserCount; ++i)
		{
...
void NetRumble::LeaderboardMenu::OnCancel()
{
	ExitScreen(false);
}

void NetRumble::LeaderboardMenu::FindLeaderboardCallBack(std::vector<std::shared_ptr<LeaderboardUser>> users)
{
