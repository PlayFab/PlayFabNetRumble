...
GamePlayScreen::GamePlayScreen() :
	GameScreen(),
	m_frameTime(0),
	m_isFailureEventReceived { false }
{
	...
	Managers::Get<GameEventManager>()->RegisterEvent(NetRumble::GameEventType::ConnectionSteamFail, this, std::bind(&GamePlayScreen::HandleServerConnectionFailureEvent, this, std::placeholders::_1));
	Managers::Get<GameEventManager>()->RegisterEvent(NetRumble::GameEventType::ConnectionRemoteTimeout, this, std::bind(&GamePlayScreen::HandleServerConnectionFailureEvent, this, std::placeholders::_1));
	Managers::Get<GameEventManager>()->RegisterEvent(NetRumble::GameEventType::PeerSentNoConnection, this, std::bind(&GamePlayScreen::HandleServerConnectionFailureEvent, this, std::placeholders::_1));
}

GamePlayScreen::~GamePlayScreen()
{
	Managers::Get<GameEventManager>()->RemoveEvent(NetRumble::GameEventType::ConnectionSteamFail, this);
	Managers::Get<GameEventManager>()->RemoveEvent(NetRumble::GameEventType::ConnectionRemoteTimeout, this);
	Managers::Get<GameEventManager>()->RemoveEvent(NetRumble::GameEventType::PeerSentNoConnection, this);
}
...
void GamePlayScreen::DrawHud(float elapsedTime)
{
	...
	if (m_isFailureEventReceived)
	{
		...
		{
			XMFLOAT2 connectFailMessagePosition = XMFLOAT2(viewportWidth / 2.0f, viewportHeight * 0.4f);

			XMVECTOR connectFailMessageLen = m_playerFont->MeasureString(DX::ChsToUtf8(m_connectFailInGameMessage).c_str());
			XMFLOAT2 connectFailMessageOrigin = XMFLOAT2(XMVectorGetX(connectFailMessageLen) / 2.0f, m_playerFont->GetLineSpacing());
			renderContext->DrawString(m_playerFont, m_connectFailInGameMessage, connectFailMessagePosition, g_game->GetWinningColor(), 0.0f, connectFailMessageOrigin, playerNameScale);
		}
	}
	renderContext->End();
}

void GamePlayScreen::HandleServerConnectionFailureEvent(GameEventMessage message)
{
	m_countdownTimer = 5.0f;
	m_isFailureEventReceived = true;
