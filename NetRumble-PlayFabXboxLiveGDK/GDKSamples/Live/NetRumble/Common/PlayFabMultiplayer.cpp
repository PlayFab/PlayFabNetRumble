...
void PlayFabOnlineManager::MultiplayerSDKInitialize()
{
	...
	if (m_pfMultiplayerHandle == nullptr)
	{
		...
		const HRESULT hr = PFMultiplayerInitialize(NETRUMBLE_PLAYFAB_TITLE_ID.data(), &m_pfMultiplayerHandle);
		if (FAILED(hr))
		{
	...
	const HRESULT hr = PFMultiplayerSetEntityToken(m_pfMultiplayerHandle, &localUserEntityKey, playFabLogin.GetEntityToken().c_str());
	if (FAILED(hr))
	{
